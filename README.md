...
| 브러시 크기 | **F 누른 채 상하로 움직이기** — 위 = 커짐, 떼면 확정, Shift = 미세 조정, ESC = 취소 |
| 컬러 팔레트 | **Q** — 전경색 피커·팔레트 팝업, 다시 **Q** 로 닫기 |
| 브러시 툴 복귀 | **B** |

### 파츠 Isolate
N패널 → Paint System → **Parts (Isolate)**: 머리·팔·몸통처럼 면을 파츠로 등록해 두고 하나만 보이게 해서 칠합니다. 숨긴 면은 칠해지지도, 앞을 가리지도 않아 가려진 부분까지 칠할 수 있습니다.
- 등록: 편집 모드(또는 면 선택 마스킹)에서 면 선택 → **+** / 분리된 메쉬 조각마다 자동 생성 → **From Loose Parts**
- 전환: **숫자키 1~9** = n번째 파츠 Isolate (같은 번호 다시 = 전체 표시), **0** = 전체 표시, 뷰포트 우측 내비게이션 버튼 아래의 번호 버튼 클릭도 동일
- 면 숨김 + 면 선택 마스킹만 사용 — 셰이더를 건드리지 않아 페인팅 속도에 영향 없음

### Art Brush Pack

N패널 → Paint System → **Brush → Art Brushes**, 또는 뷰포트 우측 내비게이션 버튼 아래의 **브러시 썸네일 버튼**(클릭 → 썸네일 그리드): 썸네일을 고르면 **현재 브러시에 프리셋이 덮어써집니다.** 기본형 4종 + 드로잉 5종 + 회화 6종 + 질감 6종, 모두 21종.
...
- **Export / Import** — 레이어 스택 ↔ PSD (이름·순서·블렌드 모드·불투명도·표시 상태 보존, 픽셀 레이어만)
  ...
  - Merge/Sync 로 기존 레이어를 덮어쓰면 레이어마다 undo 스텝이 하나씩 남는다 (Ctrl+Z 한 번 = 레이어 한 장 되돌리기)
- **Open PS** — 연동된 PSD를 포토샵에서 즉시 열기
- **Sync** — 마지막 동기화 이후 바뀐 쪽을 반대쪽에 반영하는 양방향 동기화 (자동 감시 없음, 버튼을 누를 때만)
  - 포토샵에서 저장했으면 PSD → Blender (이름이 같은 레이어는 픽셀 갱신, 새 레이어는 추가)
  - 블렌더에서 칠했으면 Blender → PSD (포토샵에 열려 있던 문서는 다시 열어야 반영)
  - 양쪽 모두 바뀌었으면 어느 쪽을 살릴지 묻는다

주의: 조정 레이어·텍스트·스마트 오브젝트·레이어 스타일은 왕복 시 보존되지 않습니다. 포토샵 전용 기능은 포토샵에서만 쓰고, 블렌더에서는 픽셀 레이어만 수정하는 운용을 권장합니다.

### Projection Tex (뷰 투사)
N패널 → **Projection Tex**: 2D 이미지(JPG/PNG)를 현재 뷰 화면 그대로 모델에 투사합니다 (병 위의 라벨 등).
1. **Import**로 이미지 등록 (다중 선택 가능, 썸네일 클릭 → 팝업에서 선택, 원본 파일 수정 시 자동 리로드, **Remove**로 제거)
2. **Place** — 뷰포트에 오버레이 표시: 드래그 이동, 휠 크기 조절 (뷰 고정)
